...
<div class="rpt-period">
  Từ ngày: <?= date('d/m/Y', strtotime($dateFrom)) ?> &nbsp;Đến ngày: <?= date('d/m/Y', strtotime($dateTo)) ?>
</div>
...
<table>
  ...
  <tbody>
    ...
    <tr><td colspan="8" class="text-center" style="padding:12px;color:#888;">Không có đơn hàng trong kỳ này</td></tr>
    <?php else: ?>
      <?php
        $i = 1;
        foreach ($orders as $o):
          ...
          $cost        = $o->cost_total ?? 0;
          $lineProfit  = isset($o->cost_total) ? ($o->total_amount - $o->cost_total) : '';
          $pmLabel     = ['cash'=>'Tiền mặt','credit_card'=>'Thẻ','bank_transfer'=>'CK', 'vnpay' => 'VNPay'][$o->payment_method] ?? ucfirst($o->payment_method);
          $customerName = $o->user ? $o->user->name : ($o->customer ? $o->customer->name : 'Khách lẻ');
      ?>
      <tr>
        ...
        <td class="text-right"><?= number_format($o->total_amount, 0, ',', '.') ?></td>
        <td class="text-right"><?= $lineProfit !== '' ? number_format($lineProfit, 0, ',', '.') : '–' ?></td>
      </tr>
      <?php endforeach; ?>
    <?php endif; ?>
  </tbody>
  <!-- TỔNG CỘNG row -->
  <tr class="tfoot-row">
    <td colspan="4" class="text-center" style="font-weight:bold;">TỔNG CỘNG:</td>
...
<div class="notes">
  Lưu ý:
  <p>• Doanh thu = Tổng tiền thu từ khách hàng trong kỳ báo cáo (đối với đơn hàng Đã Giao).</p>
  <p>• Lợi nhuận = Doanh thu – Chi phí nhập hàng (giá vốn). Giá trị âm thể hiện lỗ.</p>
  <p>• Số liệu chỉ bao gồm đơn hàng đã hoàn thành trong khoảng thời gian được chọn.</p>
</div>
