...
// Hàm chuyển hướng sang bộ sưu tập Chill khi bấm vào banner lớn
const goToChillCollection = () => {
  window.location.href = '/collection?type=chill'
}

// 1. CHUYỂN ĐỔI DỮ LIỆU THÀNH MẢNG SẢN PHẨM CHI TIẾT
const chillProducts = ref([
  { id: 2, name: 'Combo "Good Morning": Polo Sọc & Jean Baggy', price: '320.000đ', image: '/images/ChillTeamSection/f2.png' },
  { id: 3, name: 'Outfit "Bắt Trọn Ánh Nhìn": Cardigan Đỏ & Quần Suông', price: '350.000đ', image: '/images/ChillTeamSection/f3.png' },
  { id: 4, name: 'Set "Cà Phê Bệt": Thun Basic & Baggy Jean Bụi Bặm', price: '290.000đ', image: '/images/ChillTeamSection/f4.png' },
  { id: 5, name: 'Combo "Lượn Lờ Phố Xá": Sơ Mi Khoác & Quần Dù Thụng', price: '410.000đ', image: '/images/ChillTeamSection/f5.png' },
  { id: 6, name: 'Outfit "Hẹn Hò Tinh Tế": Layering Sweater Quàng Vai', price: '380.000đ', image: '/images/ChillTeamSection/f6.png' },
  { id: 7, name: 'Set "Núp Dưới Mưa": Hoodie Oversize & Jean Sáng Màu', price: '310.000đ', image: '/images/ChillTeamSection/f7.png' },
  { id: 8, name: 'Combo "Trầm Tính": Sweater Xám Layer & Quần Suông', price: '270.000đ', image: '/images/ChillTeamSection/f8.png' },
  { id: 9, name: 'Outfit "Năng Động Cuối Tuần": Polo Xanh Lá & Short Kaki', price: '330.000đ', image: '/images/ChillTeamSection/f9.png' },
  { id: 10, name: 'Set "Cool Boy Ngầm": Sweater Graphic Đen', price: '460.000đ', image: '/images/ChillTeamSection/f10.png' },
  { id: 11, name: 'Combo "Chàng Trai Ấm Áp": Thun Trắng & Áo Vắt Vai', price: '340.000đ', image: '/images/ChillTeamSection/f11.png' }
])

// 2. BIẾN QUẢN LÝ TRẠNG THÁI MODAL VÀ SIZE
    ...
    <div class="max-w-[1400px] mx-auto px-4 md:px-10">
      ...
      <div class="mb-14 text-center md:text-left">
        <h2 class="text-3xl md:text-5xl font-light uppercase text-slate-900 tracking-wide mb-4">
          TEAM CHILL <span class="text-[#705847]">ĐỦ NGẦU</span>
        </h2>
        <p class="text-slate-700 font-light text-base md:text-lg opacity-90 max-w-2xl">
          Trang phục thoải mái, chất liệu mềm mại, hợp cho chiều cuối tuần cà phê hoặc dạo phố cùng bạn bè.
        </p>
      </div>

      <div class="flex flex-col lg:flex-row gap-6">
        
        <div @click="goToChillCollection" class="lg:w-1/3 relative group cursor-pointer overflow-hidden rounded-2xl shadow-sm h-[600px]">
          <img src="/images/ChillTeamSection/f1.png" alt="Team Chill" class="w-full h-full object-cover transform group-hover:scale-105 transition duration-700 z-0">
          ...
          <div class="absolute bottom-0 left-0 w-full p-8 text-white flex flex-col justify-end z-20">
             <h3 class="font-medium text-xl mb-1 tracking-wide">TEAM CHILL</h3>
             <h4 class="font-light text-sm opacity-80 mb-4">(THE CASUAL / THE WEEKENDER)</h4>
             <p class="text-sm text-gray-200 font-light mb-6 leading-relaxed">
               Áo thun mềm, quần jogger êm, phong cách nhẹ nhàng mà vẫn chất, mỗi ngày đều thoải mái.
             </p>
             <button class="border border-white/70 bg-white/10 backdrop-blur-sm group-hover:bg-white group-hover:text-slate-900 text-white text-xs font-medium py-3 px-6 rounded-full transition-all duration-300 uppercase w-fit tracking-wider">
               Mặc Chill Tự Tin Ngay
             </button>
          </div>
        </div>

        <div class="lg:w-2/3 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
          <div v-for="product in chillProducts" :key="product.id" @click="openProductModal(product)" class="aspect-[2/3] rounded-2xl overflow-hidden cursor-pointer hover:-translate-y-2 shadow-sm hover:shadow-md transition-all duration-300 border border-white/50 relative group">
  ...
  <div v-if="isModalOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-3xl shadow-2xl max-w-4xl w-full p-8 md:p-12 overflow-hidden flex flex-col md:flex-row gap-8 md:gap-12 z-10">
      ...
      <div class="md:w-1/2 flex flex-col justify-center">
        <span class="text-sm text-[#705847] mb-2 uppercase tracking-widest font-light">Vibe Cuối Tuần</span>
